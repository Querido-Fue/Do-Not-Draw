...
- 연출 내용의 최종 권위는 `C:/Users/choo7/Downloads/DO_NOT_DRAW_연출상세기획.txt`다. 다만 사용자가 대화에서 명시적으로 덮어쓴 항목은 문서보다 우선한다. 현재 공간 미술은 문서의 꽃무늬 벽/3×4m 지시 대신 **밝고 섬뜩한 백룸 스타일/6×4.8m**를 사용한다. 벽은 무늬가 약한 황색 벽지, 바닥은 낡은 베이지 카펫, 천장은 0.6m 격자 드롭 실링과 반복 형광등으로 구성한다.
...
- `ClosedRoomStoryDirector`는 오프닝 입력 잠금, 암전 전환, 문/스위치 상호작용, 창문 환영, 추적 실루엣, 회전 판정, 탈출과 엔딩을 신호 기반으로 실행한다. 방 크기에 의존하는 플레이어·위협·조명 지점은 씬에 직접 하드코딩하지 않고 `DetailedRoomSetRefs` 마커를 통해 전달한다. 추적 실루엣의 시각 메시에는 `Assets/ExternalModels/BackroomsEntity/BackroomsEntity.prefab`을 사용하며, 빌더가 모든 서브메시에 공용 실루엣 재질을 적용하고 충돌체를 제거한다.
