...
4. `CardDeckPresenter`는 카드 생성, 이동/뒤집기, 덱 높이, 배치, 카드·음성 효과음을 담당한다. 카드 공개 신호는 착지 시점에 보내고, 음성이 끝날 때까지 다음 드로우를 잠근다. 전환 연출은 프레젠터를 명시적으로 잠글 수 있으며, 엔딩처럼 전역 드로우 번호는 유지하되 다음 카드의 화면 배치만 첫 장 기준으로 한 번 초기화할 수 있다.
...
- 최종 효과음의 권위 소스는 `Assets/Sounds/01_fluorescent_buzz_loop.wav`부터 `20_fluorescent_starter_tick.wav`까지의 기존 번호형 파일군과 몬스터 강화용 `26_entity_growl_low_loop.wav`~`31_heartbeat_climax_140to220bpm_distorted.wav`다. `32_window_face_lunge_0_800_pitch.ogg`는 창문 카드 공개 0.2초 뒤 얼굴 돌출, `33_final_exit_face_infestation_0_900_pitch.ogg`는 최종 출구 개방·얼굴 도배 시작 전용이다. 신규 26~33은 44.1kHz, `DecompressOnLoad`, 오디오 데이터 프리로드로 임포트한다. #9는 29번 다중 존재 루프를 유지하되 화면 얼굴이 렌더링될 때 `28_face_glitch_stab.wav`를 재생하지 않는다. 낮은 음에서 높은 음으로 상승하는 `30_climax_riser_glitch_hardcut.wav`는 에셋만 보존하고 런타임 참조와 전용 `Climax Riser Audio` 소스를 제거하며, 클라이맥스에는 그로울과 심장박동만 유지한다. #18은 기존 압박 사운드에 33번 개방 효과음을 추가한다. 실제 의미별 연결은 `ClosedRoom.unity`의 `ClosedRoomStoryDirector`, 문·스위치·카드·오버레이 컴포넌트에 직렬화한다. 플레이어 발소리는 첫 방의 `12_carpet_footstep_single`과 두 번째 방의 `12b_carpet_footstep_room2_pitched`를 `ClosedRoomStoryDirector`가 방 경계에서 전환하며, 뒤쪽 4연속 발소리는 `13_footsteps_behind_4steps_carpet`을 별도 큐로 사용한다.
...
- 최종 출구 트리거를 통과해 엔딩 단계에 들어가면 `ClosedRoomStoryDirector`가 현재 1인칭 화면 위로 1초 동안 흰 화면을 페이드인한다. 완전히 흰 화면 뒤에서 플레이어 렌더러 숨김, 방·덱 초기화, 프레젠터 교체와 `Ending Top View Camera` 전환을 원자적으로 처리한 뒤 1초 동안 페이드아웃한다. 탑뷰 카메라 이동은 흰색 페이드아웃이 시작되는 프레임에 함께 시작한다. 카메라 위치는 `(0, 2.72, 0.92)`, 초기 직교 크기는 `2.65`, 줌 목표는 `1.45`로 두어 첫 방 테이블 중심 `z≈0.905`와 카드 중심 `z≈0.945`가 화면 중앙 부근에 오도록 한다. 엔딩 카드 직전에는 덱 비주얼을 다시 만들고 다음 카드의 배치 인덱스만 한 번 `0`으로 초기화해 전역 드로우 번호와 카드 아트는 유지하면서 첫 장처럼 뒤집히게 한다. 플레이어 루트 아래의 렌더러는 원래 활성 상태와 함께 캐시해 캡슐을 숨기고, 초기화·컴포넌트 비활성화 시 정확히 복원한다. 엔딩 준비 시 이동 컨트롤러와 `RandomFootstepPlayer`를 함께 비활성화하고 재생 중 발소리를 즉시 정지한다. 카메라 이동 시작부터 직교 크기를 줄이는 덱 줌과 마지막 1초 검은 암전을 포함해 총 5초가 지나면 빌드에서는 `Application.Quit`, 에디터에서는 Play Mode 종료를 실행한다.
