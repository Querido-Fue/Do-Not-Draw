...
- 두 방의 `First Room Window`/`Second Room Window`에는 커튼을 두지 않으며 네 창틀 모두 책상 상판과 같은 `DeskWood.mat`를 사용한다. `Black Glass`는 실제 복도 지오메트리 대신 `ImpossibleWindowVoid.shader`의 시점 패럴랙스·뒤틀린 중첩 프레임으로 불가능한 깊이를 표현해 인접 방을 침범하지 않는다. 각 창문의 `ImpossibleWindowWatcher`는 메인 카메라가 창을 직접 응시할 때만 `Textures/WindowApparitions/WindowWatcherFace.png` 원본을 가산 합성으로 천천히 나타냈다가 사라지게 하며, 얼굴 콘텐츠를 별도 변형 이미지로 교체하지 않는다. 창문 속 얼굴의 최대 셰이더 알파는 `0.55`로 제한해 암부 공간 안에 묻히도록 한다.
...
- `ClosedRoomStoryDirector`는 3600K 즉시 하드 스타트, 카드마다 0.2초/15~20% 조명 딥과 형광등 틱, 암전 전환, 문/스위치 상호작용, 창문 환영, 추적 실루엣, 회전 판정, 탈출과 엔딩을 신호 기반으로 실행한다. #3/#4/#5는 미준수 시 같은 단계로 되돌아가 같은 카드를 다시 뽑으며, 플레이어 대신 조명·문·위치를 강제로 바꾸는 폴백은 두지 않는다. #3의 실제 끄기→켜기 뒤에는 형광등을 3400K로 복구한다. #9에서는 직접광을 22.5%, 천장 발광을 24%로 낮추고 미세 플리커 진폭을 구간 한정으로 높인다. #9 공개부터 다음 카드 뽑기 전까지 화면 주변부 얼굴을 2~4초 뒤 처음 표시하고 이후 불규칙하게 반복하며, 시간이 지날수록 빈도와 중앙 접근도를 높이되 카드 입력과 결과에는 영향을 주지 않는다. #18은 문 개방 후 5.5초 동안 문 쪽 이동이 없을 때만 7초 압박 사이클을 시작하고, 접근 이동이 감지되면 진행 중에도 즉시 해제한다. 압박 사이클은 얼굴 지수 확대, 불규칙 조명 스트로브, 붉은 비네트, 심장박동·라이저를 결합하고 하드컷 뒤 0.42초 정적과 문 쪽 바람 강조를 반복한다. 카메라 방향을 강제로 회전시키지 않으며 하드컷은 짧은 FOV 충격만 사용한다. 방 크기에 의존하는 플레이어·위협·조명 지점은 `ClosedRoom.unity`의 마커 Transform과 디렉터 직렬화 참조로 전달하므로 에디터에서 직접 조정한다. 추적 실루엣의 시각 메시에는 `Assets/ExternalModels/BackroomsEntity/BackroomsEntity.prefab`을 사용하며, 씬에 배치된 인스턴스는 모든 서브메시에 공용 실루엣 재질을 적용하고 충돌체를 제거한 상태로 보존한다.
...
- 최종 효과음의 권위 소스는 `Assets/Sounds/01_fluorescent_buzz_loop.wav`부터 `20_fluorescent_starter_tick.wav`까지의 기존 번호형 파일군과 몬스터 강화용 `26_entity_growl_low_loop.wav`~`31_heartbeat_climax_140to220bpm_distorted.wav`다. 신규 26~31은 44.1kHz, `DecompressOnLoad`, 오디오 데이터 프리로드로 임포트하며 `HorrorPerceptionOverlay`의 2D 전용 소스에 직렬화한다. #9는 29번 다중 존재 루프와 위치 팬을 적용한 28번 얼굴 스탭을 사용하고, #18은 26번 그르렁 루프·30번 7초 하드컷 라이저·31번 가속 왜곡 심장박동과 28번 스탭을 사용한다. 실제 의미별 연결은 `ClosedRoom.unity`의 `ClosedRoomStoryDirector`, 문·스위치·카드·오버레이 컴포넌트에 직렬화한다. 플레이어 발소리는 첫 방의 `12_carpet_footstep_single`과 두 번째 방의 `12b_carpet_footstep_room2_pitched`를 `ClosedRoomStoryDirector`가 방 경계에서 전환하며, 뒤쪽 4연속 발소리는 `13_footsteps_behind_4steps_carpet`을 별도 큐로 사용한다.
